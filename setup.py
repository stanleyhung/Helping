...
CREATE table INTERVIEWS (season varchar(200), jobnum integer, scheduler varchar(200), partnum varchar(200), interviewType VARCHAR(100), interviewDate Date, interviewTime Time, interviewEndTime Time, organization VARCHAR(400), jobtitle	VARCHAR(400), jobtype VARCHAR(400),	function VARCHAR(400), ucid integer, program VARCHAR(200), roundD varchar(200),	industry VARCHAR(200));
...
SELECT COUNT(*) FROM INTERVIEWS A INNER JOIN OFFERS B ON A.ucid = B.ucid AND A.season = B.season AND A.jobtype = B.jobtypeoffer AND A.organization = B.organization

CREATE TABLE interesting AS SELECT A.season, B.season, A.jobnum, A.interviewDate, A.interviewTime, A.organization, A.jobtitle, B.jobfunction, A.jobtype, B.jobtypeoffer, B.ucid 
FROM INTERVIEWS A INNER JOIN OFFERS B ON 
A.ucid = B.ucid AND A.organization = B.organization AND NOT A.season = B.season;


SELECT A.season, B.season, A.interviewDate, B.offerdate, A.jobtitle, B.jobfunction, A.jobtype, B.jobtypeoffer
FROM INTERVIEWS A INNER JOIN OFFERS B ON 
A.ucid = B.ucid AND A.organization = B.organization AND NOT A.season = B.season;
...
SELECT A.ucid, A.organization, A.season, B.season, A.interviewDate, B.offerdate, A.jobtitle, B.jobfunction, A.jobtype, B.jobtypeoffer
FROM INTERVIEWS A INNER JOIN OFFERS B ON
A.ucid = B.ucid AND A.organization = B.organization AND NOT A.season = B.season AND A.jobtype = 'Summer Internship' AND NOT B.jobtypeoffer = 'Full-Time';



SELECT season, offerdate, jobfunction, jobtype, organization FROM OFFERS WHERE ucid = 387042;
